...
test("Build: the morpheme display option explicitly prepends the morpheme", async ({ page }) => {
	...
	const label = await page.locator(".blocklyFlyout .blocklyDraggable text").filter({ hasText: /^-nngit/ }).textContent();
...
test("Build: filtering by -nngit surfaces the ordinary negator by its Kalaallisut form", async ({ page }) => {
	...
	await expect(page.locator(".blocklyFlyout .blocklyDraggable text").filter({ hasText: /^-nngit/ })).toBeVisible();
});
